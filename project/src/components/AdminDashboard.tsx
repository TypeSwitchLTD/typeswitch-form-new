...
const getSegmentScore = (data: SurveyResponse[], segment: string) => {
  const segmentData = segment === 'adhd'
    ? data.filter(r => Array.isArray(r.diagnosis) && r.diagnosis.includes('adhd'))
...
const AdminDashboard: React.FC<DashboardProps> = ({ onLogout }) => {
  ...
  useEffect(() => {
    ...
    if (filters.language !== 'all') {
      filtered = filtered.filter(r => Array.isArray(r.languages) && r.languages.includes(filters.language));
    }
    ...
    if (filters.diagnosis !== 'all') {
      if (filters.diagnosis === 'adhd') {
        filtered = filtered.filter(r => Array.isArray(r.diagnosis) && r.diagnosis.includes('adhd'));
      } else if (filters.diagnosis === 'none') {
        filtered = filtered.filter(r => !Array.isArray(r.diagnosis) || r.diagnosis.length === 0 || r.diagnosis.includes('none'));
      }
    }
  ...
  const stats = useMemo(() => {
    ...
    const withADHD = data.filter(r => Array.isArray(r.diagnosis) && r.diagnosis.includes('adhd')).length;
    const otherLearning = data.filter(r =>
      Array.isArray(r.diagnosis) && r.diagnosis.some(d => d !== 'adhd' && d !== 'none' && d !== 'undiagnosed')
    ).length;
  ...
  const adhdInsight = useMemo(() => {
    const adhd = data.filter(r => Array.isArray(r.diagnosis) && r.diagnosis.includes('adhd'));
    const nonAdhd = data.filter(r => !Array.isArray(r.diagnosis) || !r.diagnosis.includes('adhd'));
                  ...
                  <div className="bg-white rounded-lg shadow p-6">
                    ...
                    <div className="space-y-3">
                      {['Hebrew-English', 'Russian-English', 'Arabic-English'].map(lang => {
                        const count = data.filter(r => Array.isArray(r.languages) && r.languages.includes(lang)).length;
                        const percent = data.length > 0 ? Math.round((count / data.length) * 100) : 0;
                        return (
                          <div key={lang}>
                            <div className="flex justify-between text-sm mb-1">
                ...
                <div className="bg-gradient-to-r from-red-600 to-orange-600 rounded-lg p-6 text-white">
                  ...
                  <div className="space-y-3">
                    {(() => {
                      ...
                      const avoidance = data.filter(r => Array.isArray(r.awakening_symptoms) && r.awakening_symptoms.includes('avoid_multilingual')).length;
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="bg-yellow-50 border-2 border-yellow-400 rounded-lg p-6">
                    ...
                    <div className="space-y-2 text-sm">
                      {(() => {
                        const checksIcon = data.filter(r => Array.isArray(r.awakening_symptoms) && r.awakening_symptoms.includes('glance_icon')).length;
                        const extraShortcut = data.filter(r => Array.isArray(r.awakening_symptoms) && r.awakening_symptoms.includes('extra_shortcut')).length;
                        return (
                          <>
                  ...
                  <div className="bg-red-50 border-2 border-red-400 rounded-lg p-6">
                    ...
                    <div className="space-y-2 text-sm">
                      {(() => {
                        const deleteWord = data.filter(r => Array.isArray(r.awakening_symptoms) && r.awakening_symptoms.includes('delete_word')).length;
                        const sentWrong = data.filter(r => Array.isArray(r.awakening_symptoms) && r.awakening_symptoms.includes('sent_wrong_lang')).length;
                        return (
                          <>
                  ...
                  <div className="bg-orange-50 border-2 border-orange-400 rounded-lg p-6">
                    ...
                    <div className="space-y-2 text-sm">
                      {(() => {
                        const avoidDocs = data.filter(r => Array.isArray(r.awakening_symptoms) && r.awakening_symptoms.includes('avoid_multilingual')).length;
                        const separateApps = data.filter(r => Array.isArray(r.awakening_symptoms) && r.awakening_symptoms.includes('use_separate_apps')).length;
                        return (
                          <>
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {paginatedData.map((response) => (
                          <tr key={response.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-4 py-3">
                              {Array.isArray(response.diagnosis) && response.diagnosis.includes('adhd') && (
